...
def structure_factor_db_density(
    r_flat: np.ndarray,
    k_vals: np.ndarray,
...
def _compute_single_trajectory_sk(args: tuple[Any, ...]) -> dict[str, Any]:
    ...
    try:
        ...
        sk_values = structure_factor_db_density(
            coords,
            k_vals,
            box=box,
...
def compute_static_structure_factor_from_files(
    *,
    ...
    raw_k = k_vectors(float(Lx), float(Ly), float(Lz), float(k_max))
    if raw_k.size == 0:
    ...
    print("=" * 60, flush=True)
    print("STATIC STRUCTURE FACTOR CALCULATION", flush=True)
    print("=" * 60, flush=True)
    ...
    print(f"\nSaved S(k) output: {resolved_output_file}", flush=True)
